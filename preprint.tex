...
can eat members of their own species (i.e.~species having self loops),
...
network structure than the neutral model for all measures considered.
...
biological traits. Therefore, these results show that having information
on the number of prey and predators for each species substantially
improves the prediction of food-web structure, both compared to models
solely based on connectance and the ones solely based on species
relative abundances.
...
(\(0.73 \pm 0.05\); empirical networks: \(0.63 \pm 0.09\)) for all
networks in our complete dataset (tbl.~\ref{tbl:measures_all}). This
might in part be due to the fact that nestedness was calculated using
the spectral radius of the adjacency matrix, which directly leverages
information on the network itself just like the heuristic MaxEnt models.
...
models. This suggests that, although the joint degree sequence is an
important driver of food-web structure, other ecological constraints
might need to be added to our MaxEnt model to account for some emerging
food-web properties, especially the ones regarding food-chain lengths.
...
for networks with high empirical values. In fig.~S3, we show that the
...
entropy considering a fixed joint degree sequence (fig.~S4). Empirical
...
number of interactions, or connectance (fig.~S5). Similarly, we found no
...
community dynamics and assembly (Stouffer and Bascompte 2011). In
fig.~\ref{fig:motifs}, we show that the motifs profile of networks
generated using the type II MaxEnt network model was very close to the
one of empirical data, and that they made better predictions than the
type II null model. This is also shown in fig.~\ref{fig:motifs_rel},
where we see that relationships between motifs pairs of empirical food
webs are very similar to the ones of the type II MaxEnt networks.
Overall, these results suggest that the type II MaxEnt network model can
reproduce many aspects of network structure. This highlights the
importance of the joint degree sequence in shaping ecological networks.
...
the entire joint degree sequence, i.e.~on the number of prey and
...
marginal numbers of predators and prey.
...
with the entire chains (or part of the chains).
...
data. We found that the numbers of prey and predators for each species
