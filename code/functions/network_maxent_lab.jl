...
function network_maxent(N::UnipartiteNetwork, nsteps::Int64)
    ...
    # initial vector for SVD-entropy
    entropies = zeros(Float64, nsteps)
    entropies[1] = svd_entropy(A0)
    ...
    for i in 2:nsteps
        ...
        entropies[i] = svd_entropy(A0)
        # update the temperature
        T = T * 0.99
    end
    max_entropy = svd_entropy(A0)
    Amax = (A = A0, entropy = max_entropy, entropies = entropies)
    return Amax
end
...
maxent_N = network_maxent(N, nsteps)
plot(maxent_N.entropies, label="", xlab="step", ylab="SVD-entropy")
